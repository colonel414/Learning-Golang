...
func main() {
	...
	s = make([]string, 3)
	// makeSlice := make([]int, 3, 5) // slice of length 3, capacity 5
	...
	slice = append(slice, 6, 7, 8) // slice = [2, 3, 4, 6, 7, 8]

	// Modifying elements in a slice
	slice[0] = 10 // slice = [10, 3, 4, 6, 7, 8]
	...
	l := s[2:5]
	fmt.Println("slicop:", l)

	// This slices up to (but excluding) s[5].
	l = s[:5]
	fmt.Println("slicop2:", l)

	// And this slices up from (and including) s[2].
	l = s[2:]
	fmt.Println("slicop3:", l)
	...
	// The length of the inner slices can vary, unlike with multidimensional arrays.
	twoD := make([][]int, 3)
	for i := 0; i < 3; i++ {
		innerLen := i + 1
